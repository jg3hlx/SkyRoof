...
namespace VE3NEA
{
  /// <summary>Radio selection in OmniRig.</summary>
  ...
  /// <summary>The arguments of the CustomReply event.</summary>
  public sealed class CustomReplyArgs : EventArgs
  {
    /// <summary>Gets the reply bytes array.</summary>
    /// <value>Reply from the radio.</value>
    public byte[] ReplyBytes{ get; }

    /// <summary>Initializes a new instance of the <see cref="CustomReplyArgs" /> class.</summary>
    /// <param name="replyBytes">The reply bytes.</param>
    public CustomReplyArgs(byte[] replyBytes)
  ...
  public class OmniRigClient
  {
    ...
    /// <summary>Occurs when the radio replies to a custom command.</summary>
    public event EventHandler<CustomReplyArgs> CustomReply;
    ...
    private void Start()
    {
      try
      {
        ...
        events.CustomReply += CustomReplyhandler;
      }
      catch
      {
        Stop();
  ...
  private void StatusChangedEventHandler(int rigNo)
    ...
    public void SendCustomCommand(byte[] commandBytes, int replyLength)
    {
      rig?.SendCustomCommand(commandBytes, replyLength, "");
    }

    private void CustomReplyhandler(int RigNumber, object Command, object Reply)
    {
      if (RigNumber != RigNo) return;

      context.Post(s => OnCustomReply((byte[])Reply), null);
    }


    private void OnCustomReply(byte[] reply)
    {
      CustomReply?.Invoke(this, new CustomReplyArgs(reply));
    }
